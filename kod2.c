...
void startGame() {
	...
	while(1) {
		...
		if (kanal >= pozycjaD && pozycjaD < 100) {
			if(kanal/10 != pozycjaD/10) {
				LCDPutStr("     ",y2,pozycjaD,SMALL,WHITE,BLACK);
				pozycjaD+= 10;
				write_char_USART0('P');
				LCDPutStr("==A==",y2,pozycjaD,SMALL,WHITE,BLACK);
			}
			else pozycjaD = pozycjaD;
		}
		
		else if ( kanal <= pozycjaD && pozycjaD > 0) {
			if(kanal/10 != pozycjaD/10) {
				LCDPutStr("     ",y2,pozycjaD,SMALL,WHITE,BLACK);
				pozycjaD-= 10;
				write_char_USART0('L');
		...
		else if (pozycjaD >= 100 || pozycjaD <= 10) {
			pozycjaD = pozycjaD;
		}
...
void menu() {
  ...
  int wyborMenu = 0;
	...
	while(1) {
		...
		if((PIOA_PDSR & J_DOWN)==0 && wyborMenu < 3) {
			LCDPutStr(" ",(wyborMenu*25)+5,0,SMALL,WHITE,BLUE);
			wyborMenu+=1;
			LCDPutStr(">",(wyborMenu*25)+5,0,SMALL,WHITE,BLUE);
			delay(500);
